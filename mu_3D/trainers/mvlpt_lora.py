...
def load_clip_to_cpu(cfg):
    ...
    bootstrap_method = "inflation"  # 'centering', 'inflation', 'centering_square', '3conv', cfg.MODEL.BACKBONE.BOOTSTRAP
...
class ImageEncoder(nn.Module):
    def __init__(self, clip_model, mvlpt_model):
        ...
        #     ssf_scale, ssf_shift = init_ssf_scale_shift(d_model, dtype)
    ...
    def forward(self, x: torch.Tensor, vpt_embeddings=None, vpt_embeddings_deep=None):
        ...
        # print('ImageEncoder x shape', x.shape)  # torch.Size([32, 768, 3, 3, 3])

        x = ssf_ada(x, self.ssf_scale_1, self.ssf_shift_1)
        # print('Imageencoder', x.shape, self.visual.ssf_scale_1.data.shape, self.visual.ssf_shift_1.data.shape)
...
class MVLPT_lora_trainer(TrainerX):
    ...
    def build_model(self):
        ...
        for name, param in self.model.named_parameters():
            # if "prompt_learner" not in name and 'visual.conv1' not in name and 'visual.positional_embedding' not in name and 'visual.scale' not in name and 'visual.shift' not in name:
