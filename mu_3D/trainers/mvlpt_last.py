...
class ImageEncoder(nn.Module):
    def __init__(self, clip_model, mvlpt_model, classnames):
        ...
        # self.ssf_scale_6, self.ssf_shift_6 = init_ssf_scale_shift(width, dtype=dtype)
        # self.ssf_scale_7, self.ssf_shift_7 = init_ssf_scale_shift(width, dtype=dtype)
    ...
    def forward(self, x: torch.Tensor, vpt_embeddings=None, vpt_embeddings_deep=None, extract_layer=False):
        ...
        # TODO
        # x = ssf_ada(x, self.visual.scale, self.visual.shift).to(x.dtype)
        # x = ssf_ada(x, self.ssf_scale_1, self.ssf_shift_1)

        # x = ssf_ada(x, self.ssf_scale_2, self.ssf_shift_2)
        x = self.visual.ln_pre(x)
        # x = ssf_ada(x, self.ssf_scale_3, self.ssf_shift_3)
        ...
        # print('ImageEncoder shape', x.shape)
        x = self.head(x)
        # print('ImageEncoder head shape', x.shape)
        ######### EXTRACT FEATURE FOR EACH LAYER #########
        if extract_layer:
...
class CustomCLIP(nn.Module):
    ...
    def forward(self, image, task=None):
        ...
        # print('Custom CLip logits shape', logits.shape)
        return logits
...
class MVLPT_last_trainer(TrainerX):
    ...
    def build_model(self):
        ...
        #     # if "prompt_learner" not in name and 'visual.conv1.weight' not in name and 'visual.positional_embedding' not in name and 'visual.scale' not in name and 'visual.shift' not in name:
        #     if "prompt_learner" not in name and 'visual.scale' not in name and 'visual.shift' not in name:
