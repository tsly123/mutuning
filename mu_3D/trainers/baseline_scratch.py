...
def load_clip_to_cpu(cfg):
    ...
    if in_chans_3D is not None:
        model = clip.build_model(state_dict or model.state_dict(), in_chans_3D, bootstrap_method,
                                 depth_chans_3D, is_finetune=True)
    else:
...
class ImageEncoder(nn.Module):
    def __init__(self, clip_model, classnames):
        ...
        # self.ssf_scale_2, self.ssf_shift_2 = init_ssf_scale_shift(embed_dim, dtype=dtype)
...
class CustomCLIP(nn.Module):
    ...
    def get_intermediate_layers(self, image):
        ...
        return class_token.cpu().numpy(), torch.stack(weights_all_blocks).cpu().numpy(), torch.stack(weights_attnout_blocks).cpu().numpy()
...
class Baseline_scratch_trainer(TrainerX):
    ...
    def get_feat(self, split=None, name_model=None):
        ...
        for batch_idx, batch in enumerate(tqdm(data_loader)):
            ...
            np.savez(save_path, class_token=class_token,
                                weights_all_blocks=weights_all_blocks,
                                weights_attnout_blocks=weights_attnout_blocks)
