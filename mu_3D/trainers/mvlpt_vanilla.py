...
class ImageEncoder(nn.Module):
    def __init__(self, clip_model, mvlpt_model):
        ...
        # self.scale1 = nn.Parameter(torch.ones(width, dtype=dtype))
        # self.shift1 = nn.Parameter(torch.zeros(width, dtype=dtype))
        #
        # nn.init.normal_(self.scale1, mean=1, std=.02)
        # nn.init.normal_(self.shift1, std=.02)



        # self.scale1 = nn.Parameter(torch.ones(embed_dim, dtype=dtype))
        # self.shift1 = nn.Parameter(torch.zeros(embed_dim, dtype=dtype))
        #
        # nn.init.normal_(self.scale1, mean=1, std=.02)
        # nn.init.normal_(self.shift1, std=.02)
    ...
    def forward(self, x: torch.Tensor, vpt_embeddings=None, vpt_embeddings_deep=None, extract_layer=False):
        ...
        # x = ssf_ada(x, self.ssf_scale_6, self.ssf_shift_6)
        ...
        # x = ssf_ada(x, self.scale1, self.shift1)  # .to(x.dtype)
        x = ssf_ada(x, self.ssf_scale_1, self.ssf_shift_1)  # .type(x.type())
        ...
        # x = ssf_ada(x, self.ssf_scale_7, self.ssf_shift_7)
        ...
        if self.visual.proj is not None:
            # self.visual.proj.data = ssf_ada(self.visual.proj.data, self.ssf_scale_2, self.ssf_shift_2)#.to(x.dtype)
...
class MVLPT(TrainerX):
    ...
    def build_model(self):
        ...
        print("Turning off gradients in both the image and the text encoder")
        for name, param in self.model.named_parameters():
            # if "prompt_learner" not in name and 'visual.conv1' not in name and 'visual.positional_embedding' not in name and 'visual.scale' not in name and 'visual.shift' not in name and 'class_embedding' not in name:# and 'visual.proj' not in name:
            #     param.requires_grad_(False)
            # else:
            #     # print(name, param.shape)
            ...
            if 'visual.conv1' in name or 'visual.positional_embedding' in name or 'visual.scale' in name \
                    or 'visual.shift' in name or 'class_embedding' in name:
                param.requires_grad_(True)
    ...
    def test(self, split=None):
        ...
        # print("list result", list(results.values()))
        # acc_test = results["accuracy"]
        # auc_test = results["AUC"]
        # print(f"accuracy {acc_test}, AUC {auc_test}")
        # return list(results.values())[0], list(results.values())[-1]
