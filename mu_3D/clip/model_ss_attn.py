...
def init_ssf_scale_shift(dim):
    scale = nn.Parameter(torch.ones(dim, dtype=torch.half))
    shift = nn.Parameter(torch.zeros(dim, dtype=torch.half))
...
class ResidualAttentionBlock(nn.Module):
    ...
    def attention(self, x: torch.Tensor):
        self.attn_mask = self.attn_mask.to(dtype=x.dtype, device=x.device) if self.attn_mask is not None else None
        """
        image_encoder.visual.transformer.resblocks.0.attn.in_proj_weight torch.Size([2304, 768])
        image_encoder.visual.transformer.resblocks.0.attn.in_proj_bias torch.Size([2304])
        image_encoder.visual.transformer.resblocks.0.attn.out_proj.weight torch.Size([768, 768])
        image_encoder.visual.transformer.resblocks.0.attn.out_proj.bias torch.Size([768])
        """
        # prompt
    ...
    def forward(self, x: torch.Tensor):
        ...
        # x = x + ssf_ada(self.attention(ssf_ada(self.ln_1(x), self.ssf_scale_1, self.ssf_shift_1)),
        #                 self.ssf_scale_2, self.ssf_shift_2)

        # print('CLIP dtype 1', x.type())
        # x = ssf_ada(self.ln_1(x), self.ssf_scale_1.float(), self.ssf_shift_1.float())
        # print('CLIP dtype 2', x.type())
        # x = x.float() + ssf_ada(self.attention(x), self.ssf_scale_2.float(), self.ssf_shift_2.float())

        # x = ssf_ada(self.ln_2(x), self.ssf_scale_3, self.ssf_shift_3)
        # x = x + self.mlp(ssf_ada(self.ln_2(x), self.ssf_scale_3, self.ssf_shift_3))
        return x
