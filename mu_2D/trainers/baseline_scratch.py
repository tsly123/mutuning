...
def load_clip_to_cpu(cfg):
    ...
    model = clip.build_model(state_dict or model.state_dict(), is_finetune=True)
...
class Baseline_scratch_trainer(TrainerX):
    ...
    def check_cfg(self, cfg):
        assert cfg.TRAINER.Baseline_scratch.PREC in ["fp16", "fp32", "amp"]
    ...
    def build_model(self):
        ...
        if cfg.TRAINER.Baseline_scratch.PREC == "fp32" or cfg.TRAINER.Baseline_scratch.PREC == "amp":
        ...
        if cfg.TRAINER.Baseline_scratch.PREC == "fp16":
        ...
        self.register_model("baseline_scratch_model", self.model.image_encoder, self.optim, self.sched)
        self.scaler = GradScaler() if cfg.TRAINER.Baseline_scratch.PREC == "amp" else None
    ...
    def forward_backward(self, batch):
        ...
        prec = self.cfg.TRAINER.Baseline_scratch.PREC
        if prec == "amp":
            with autocast():
    ...
    def load_model_feat(self, directory, name_model=None):
        ...
        for name in names:
            model_path = glob.glob(f'{directory}/{name}/{name_model}')[0]
    ...
    def get_feat(self, split=None, name_model=None):
        ...
        output_dir = f"{self.cfg.get_feat}/feat/{name_model}"
        os.makedirs(output_dir, exist_ok=True)

        class_token_list = torch.zeros(1, 512).cpu()#.to(self.device)
        weights_all_blocks_list = torch.zeros(12, 1, 196+1, 768).cpu()#.to(self.device)
        weights_attnout_blocks_list = torch.zeros(12, 1, 196+1, 768).cpu()#.to(self.device)
        for batch_idx, batch in enumerate(tqdm(data_loader)):
