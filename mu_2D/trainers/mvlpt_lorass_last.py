...
from clip import clip_lora as clip
...
def load_clip_to_cpu(cfg):
    ...
    # mode: lorass_qkv, lorass_qkvattn, qkvsb, ss_qkvsb
    model = clip.build_model(state_dict or model.state_dict(), ssmode='qkvsb')
...
class ImageEncoder(nn.Module):
    ...
    def get_last_selfattention(self, x: torch.Tensor, vpt_embeddings=None, vpt_embeddings_deep=None):
        x = self.visual.conv1(x)  # shape = [*, width, grid, grid]
        # x = ssf_ada(x, self.ssf_scale_1, self.ssf_shift_1)
        ...
        if self.mvlpt_model.vpt_deep and (vpt_embeddings_deep is not None or self.mvlpt_model.vpt_embeddings_deep is not None):
            ...
            for layer_idx in range(self.visual.transformer.layers):
                ...
                if layer_idx == 0:
                    ...
                    if layer_idx < self.visual.transformer.layers - 1:
                        x = layer(x)
                    else:
                        return layer(x, return_attention=True)[1]
    ...
    def get_selfattention(self, x: torch.Tensor, vpt_embeddings=None, vpt_embeddings_deep=None):
    ...
    def get_intermediate_layers(self, x: torch.Tensor, vpt_embeddings=None, vpt_embeddings_deep=None):
        weights_all_blocks = []
        ...
        for layer_idx in range(self.visual.transformer.layers):
            ...
            if layer_idx == 0:
                # x = layer(x)
                x, attnout_weight, attnout_viz = layer(x, return_attn_weight=True, return_attention=True)
            elif layer_idx <= vpt_embeddings_deep.shape[0]:
                vpt_emb_deep = self.mvlpt_model.vpt_dropout(self.mvlpt_model.vpt_proj(
                    vpt_embeddings_deep[layer_idx - 1]).expand(B, -1, -1)).to(x.dtype)
            ...
            # x_out = nn.functional.normalize(x_out, dim=0)
            attnout_weight = attnout_weight.permute(1, 0, 2)

            slice_id = [0] + list(range(1 + self.mvlpt_model.vpt_n_ctx, x_out.shape[1]))
            # print(slice_id)
            x_out = x_out[:, slice_id, :]
            attnout_weight = attnout_weight[:, slice_id, :]

            weights_all_blocks.append(x_out)
            weights_attnout_blocks.append(attnout_weight)
            attnout_viz_blocks.append(attnout_viz)
        ...
        return class_token, weights_all_blocks, weights_attnout_blocks, attnout_viz_blocks
...
class CustomCLIP(nn.Module):
    ...
    def get_last_selfattention(self, image):
        ...
        # image_features = self.image_encoder(image.type(self.dtype), vpt_emb, vpt_emb_deep)
        attn = self.image_encoder.get_last_selfattention(image.type(self.dtype), vpt_emb, vpt_emb_deep)
        return attn

    def get_selfattention(self, image):
        coop_emb, vpt_emb, vpt_emb_deep = self.prompt_learner.forward_mvlpt_proj(self.dtype)

        # image_features = self.image_encoder(image.type(self.dtype), vpt_emb, vpt_emb_deep)
        attn = self.image_encoder.get_selfattention(image.type(self.dtype), vpt_emb, vpt_emb_deep)
        return torch.stack(attn)

    def get_intermediate_layers(self, image):
        coop_emb, vpt_emb, vpt_emb_deep = self.prompt_learner.forward_mvlpt_proj(self.dtype)

        # image_features = self.image_encoder(image.type(self.dtype), vpt_emb, vpt_emb_deep)
        class_token, weights_all_blocks, weights_attnout_blocks, attnout_viz_blocks = self.image_encoder.get_intermediate_layers(image.type(self.dtype), vpt_emb, vpt_emb_deep)
        return class_token, torch.stack(weights_all_blocks), torch.stack(weights_attnout_blocks), torch.stack(attnout_viz_blocks)
...
class MVLPT_lorass_last_trainer(TrainerX):
    ...
    def build_model(self):
        ...
        for name, param in self.model.named_parameters():
            ...
            # if coop_n_ctx != 0 and 'text_encoder' in name and 'bias' in name:
            #     param.requires_grad_(True)
            #     print(name, param.shape)
    ...
    @torch.no_grad()
    def test(self, split=None, get_output=False):
        from tqdm import tqdm
        import copy
        import numpy as np
        ...
        if get_output:
            # print(results["_y_true"])
            # print(results["_y_pred"])
            true = np.array(results["_y_true"])
            pred = np.array(results["_y_pred"])
            print('true', true.shape, true)
            print('pred', pred.shape, pred)
            if self.cfg.get_feat is not None:
                output_dir = glob.glob(f'{self.cfg.get_feat}/*{self.name_model}')[0]
                output_dir = f"{output_dir}/feat"
                # output_dir = f'{self.cfg.get_feat}/output_truepred'
            else:
                # output_dir = f'{self.cfg.OUTPUT_DIR}/output_truepred'
                return true, pred
            os.makedirs(output_dir, exist_ok=True)

            save_path = f"{output_dir}/output_{self.name_model}.npz"
            np.savez(save_path, true=true,
                                pred=pred)
            return
    ...
    def load_model_feat(self, directory, name_model=None):
        import glob
        ...
        for name in names:
            model_path = glob.glob(f'{directory}/*{name_model}/*.pth.tar*')[0]
            # print(model_path)
    ...
    @torch.no_grad()
    def get_feat(self, split=None, name_model=None):
        from tqdm import tqdm
        import glob
        import os
        """A generic testing pipeline."""
        ...
        output_dir = glob.glob(f'{self.cfg.get_feat}/*{name_model}')[0]
        output_dir = f"{output_dir}/feat"
        os.makedirs(output_dir, exist_ok=True)

        class_token_list = torch.zeros(1, 512)#.to(self.device)
        weights_all_blocks_list = torch.zeros(12, 1, 196+1, 768)#.to(self.device)
        weights_attnout_blocks_list = torch.zeros(12, 1, 196+1, 768)#.to(self.device)
        attnout_viz_blocks_list = torch.zeros(12, 1, 196, 196)
        true = np.zeros(1)
        pred = np.zeros(1)
        ...
        for batch_idx, batch in enumerate(tqdm(data_loader)):
            ...
            # output = self.model_inference(input, task=tasks_)
            # HACK: make everything one-hot vector label!
            # _true, _pred = self.test(get_output=True)
            class_token, weights_all_blocks, weights_attnout_blocks, attnout_viz_blocks = self.model.get_intermediate_layers(img)


            class_token_list = torch.cat([class_token_list, class_token.cpu()],dim=0)
            weights_all_blocks_list = torch.cat([weights_all_blocks_list, weights_all_blocks.cpu()],dim=1)
            weights_attnout_blocks_list = torch.cat([weights_attnout_blocks_list, weights_attnout_blocks.cpu()],dim=1)

            #############################
            # TODO ???
            # slice_id = [0] + list(range(1+self.cfg.TRAINER.MVLPT.VPT.N_CTX, attnout_viz_blocks.shape[-1]))
            # # print('viz slice_id', len(slice_id), slice_id)

            # attnout_viz_blocks = attnout_viz_blocks[:, :, slice_id]
            # attnout_viz_blocks = attnout_viz_blocks[:, :, :, slice_id]  # [slice_id]
            # attnout_viz_blocks = attnout_viz_blocks.view(12, -1,  len(slice_id), len(slice_id)).cpu()
            #
            # attnout_viz_blocks_list = torch.cat([attnout_viz_blocks_list, attnout_viz_blocks], dim=1)
            #############################
            # true = np.concatenate((true, _true), axis=0)
            # pred = np.concatenate((pred, _pred), axis=0)

            # class_token_list.append(class_token)
            # weights_all_blocks_list.append(weights_all_blocks)
            # weights_attnout_blocks_list.append(weights_attnout_blocks)
            # class_token_list += [class_token]
            # weights_all_blocks_list += [weights_all_blocks]
            # weights_attnout_blocks_list += [weights_attnout_blocks]


        # class_token_list = torch.cat(class_token_list,dim=0).cpu().numpy()
        # # class_token_list = class_token_list.view(class_token_list.shape[1], -1)(2, 12, 77, 197, 768)
        # weights_all_blocks_list = torch.cat(weights_all_blocks_list,dim=0).cpu().numpy()
        # weights_attnout_blocks_list = torch.cat(weights_attnout_blocks_list,dim=0).cpu().numpy()


        # print('class_token_list', class_token_list.shape)
        # print('weights_all_blocks_list', weights_all_blocks_list.shape)
        # print('weights_attnout_blocks_list', weights_attnout_blocks_list.shape)

        class_token_list = class_token_list[1:,:].cpu().numpy()
        weights_all_blocks_list = weights_all_blocks_list[:,1:,...].cpu().numpy()
        weights_attnout_blocks_list = weights_attnout_blocks_list[:,1:,...].cpu().numpy()

        # print('class_token_list', class_token_list.shape)
        # print('weights_all_blocks_list', weights_all_blocks_list.shape)
        # print('weights_attnout_blocks_list', weights_attnout_blocks_list.shape)

        # assert true.shape[0] == class_token_list.shape[0]
        # assert pred.shape[0] == class_token_list.shape[0]


        save_path = f"{output_dir}/feat.npz"
        np.savez(save_path, class_token=class_token_list,
                            weights_all_blocks=weights_all_blocks_list,
                            weights_attnout_blocks=weights_attnout_blocks_list,
                            # true=true,
                            # pred=pred
                            )
    ...
    def viz_prosess(self, img, attentions, threshold=None, patch_size=16, output_dir=None, img_id=0):
        ...
        if threshold is not None:
            ...
            for j in range(nh):
                viz.display_instances(image, th_attn[j], fname=os.path.join(output_dir, f"id{img_id}_mask_th" + str(threshold) + "_head" + str(j) + ".png"), blur=False)
    ...
    def viz(self, split=None, name_model=None):
        ...
        output_dir = f"{self.cfg.OUTPUT_DIR}/viz_savefig5/{name_model}"
        os.makedirs(output_dir, exist_ok=True)
        ...
        for batch_idx, batch in enumerate(tqdm(data_loader)):
            ...
            attn = attn[:, :, :, slice_id]  # [slice_id]
            # # print('viz 1', attn.shape)
            # attn = attn.view(12, -1, 12, len(slice_id), len(slice_id))
            attn = attn.view(12, -1,  len(slice_id), len(slice_id))
            print('viz 2', attn.shape)


            true = true_all[batch_idx*self.cfg.DATALOADER.TEST.BATCH_SIZE: (batch_idx+1)*self.cfg.DATALOADER.TEST.BATCH_SIZE]
            pred = pred_all[batch_idx * self.cfg.DATALOADER.TEST.BATCH_SIZE: (batch_idx + 1) * self.cfg.DATALOADER.TEST.BATCH_SIZE]
            ...
            for attn_i in range(attn.shape[1]):
                ...
                self.get_image_attn_mask(img_pp, img_attn_weights, img_id=img_id, output_dir=output_dir, output=(true[attn_i], pred[attn_i]))
    ...
    def get_image_attn_mask(self, img, att_mat, token=0, img_id=0, output_dir=None, output=None):
        ...
        for layer_i in range(att_mat.shape[0]):
            ...
            mask = v[token, att].reshape(grid_size, grid_size).numpy()  # token=0 is CLS (the only one used by CLIP at the end)
