...
class ImageEncoder(nn.Module):
    def __init__(self, clip_model, classnames):
        ...
        # Classifier Head
        # self.fc_norm = torch.nn.BatchNorm1d(embed_dim, affine=False, eps=1e-6)
        # self.head = nn.Linear(embed_dim, len(classnames))
        self.head = nn.Sequential(torch.nn.BatchNorm1d(embed_dim, affine=False, eps=1e-6),
                                  nn.Linear(embed_dim, len(classnames)))

        # make sure requires_grad = False
        # set bias requires_grad = False in the trainer below
...
class CustomCLIP(nn.Module):
    ...
    #
    #     # max_length = self.clip_model.context_length
    #     print("Current Context Length is: ", self.max_length)
    #
    #     tokenized_prompts = torch.cat([clip.tokenize(p, context_length=self.max_length) for p in prompts]).to('cuda')
    #     # print('device', tokenized_prompts.get_device()) # -1 == cpu
    #     # print('device', self.clip_model.get_device())   # error
    #
    #     with torch.no_grad():
    ...
    def forward(self, image, task=None):
        ...
        # prompts, tokenized_prompts = self.forward_coop()
    ...
    def get_intermediate_layers(self, image):
        class_token, weights_all_blocks, weights_attnout_blocks = self.image_encoder.get_intermediate_layers(image.type(self.dtype))
        return class_token, torch.stack(weights_all_blocks), torch.stack(weights_attnout_blocks)

    def get_selfattention(self, image):
        attn = self.image_encoder.get_selfattention(image.type(self.dtype))
...
class Baseline_sb_trainer(TrainerX):
    ...
    def check_cfg(self, cfg):
        assert cfg.TRAINER.Baseline_sb.PREC in ["fp16", "fp32", "amp"]
    ...
    def build_model(self):
        ...
        if cfg.TRAINER.Baseline_sb.PREC == "fp32" or cfg.TRAINER.Baseline_sb.PREC == "amp":
        ...
        if cfg.TRAINER.Baseline_sb.PREC == "fp16":
        ...
        self.register_model("Baseline_sb", self.model.image_encoder, self.optim, self.sched)
        self.scaler = GradScaler() if cfg.TRAINER.Baseline_sb.PREC == "amp" else None
    ...
    def forward_backward(self, batch):
        ...
        prec = self.cfg.TRAINER.Baseline_sb.PREC
        if prec == "amp":
            with autocast():
    ...
    def get_feat(self, split=None, name_model=None):
        ...
        for batch_idx, batch in enumerate(tqdm(data_loader)):
            ...
            # output = self.model_inference(input, task=tasks_)
            # HACK: make everything one-hot vector label!
            class_token, weights_all_blocks, weights_attnout_blocks = self.model.get_intermediate_layers(img)
