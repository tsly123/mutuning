...
# import datasets.braincell3

import datasets.pneumoniamnist
import datasets.dermamnist
import datasets.retinamnist
import datasets.patchcamelyon
import datasets.breastmnist
import datasets.pathmnist
import datasets.camelyon17_wild
import datasets.rxrx1
###################################

import datasets.proposal_prompt_0
import datasets.proposal_prompt_1
import datasets.proposal_prompt_2
import datasets.proposal_prompt_3
import datasets.proposal_prompt_4
import datasets.proposal_prompt_5
import datasets.proposal_prompt_6
import datasets.proposal_prompt_7
###################################
...
def copy_files(cfg):
    ...
    trainer_scr = '/project/hnguyen2/stly/code/prompting/MVLPT_2D/trainers/mvlpt_last.py'
    trainer_dst = trainer_dir+'/mvlpt_last.py'
    shutil.copyfile(trainer_scr, trainer_dst)

    clip_model_scr = '/project/hnguyen2/stly/code/prompting/MVLPT_2D/clip/model.py'
    clip_model_dst = trainer_dir + '/clip_model.py'
    shutil.copyfile(clip_model_scr, clip_model_dst)

    cfg_scr = '/project/hnguyen2/stly/code/prompting/MVLPT_2D/configs/trainers/MVLPT/vit_b16.yaml'
    cfg_dst = trainer_dir + '/vit_b16.yaml'
    shutil.copyfile(cfg_scr, cfg_dst)

    cfg_scr = '/project/hnguyen2/stly/code/prompting/MVLPT_2D/train_mvlpt_clip/train_mvlpt_last.py'
    cfg_dst = trainer_dir + '/train_mvlpt_last.py'
    shutil.copyfile(cfg_scr, cfg_dst)
...
def extend_cfg(cfg):
    ...
    # cfg.TRAINER.Baseline.PROJECT_METHOD = 'transformer' # could be identity / mlp / transformer
    # cfg.TRAINER.Baseline.PROJECT_DIM = 128 # if coop/vpt dimension doesnot match, project to vpt/coop
...
def main(args):
    ...
    if args.viz_attn != "None":
        ...
        print('Visualize attention map with ckpt path', args.viz_attn)
        assert os.path.isdir(args.viz_attn)
        print('\nVisualize with best val')
        trainer.load_model(args.viz_attn, name_model='model-best.pth.tar')
        trainer.viz(name_model='model-best.pth.tar')

        print('\nVisualize with epoch 200')
        trainer.load_model(args.viz_attn, name_model='model.pth.tar-200')
        trainer.viz(name_model='model.pth.tar-200')

        print('\nVisualize with best AUC')
        trainer.load_model(args.viz_attn, name_model='model-best-auc.pth.tar')
        trainer.viz(name_model='model-best-auc.pth.tar')

        print('\nVisualize with best balance')
        trainer.load_model(args.viz_attn, name_model='model-best-balance.pth.tar')
        trainer.viz(name_model='model-best-balance.pth.tar')

        print('\nVisualize with best ACC with AUC > 50')
        trainer.load_model(args.viz_attn, name_model='model-best-50auc.pth.tar')
        trainer.viz(name_model='model-best-50auc.pth.tar')
        return
    ...
    #     print('Eval with best val')
    #     trainer.load_model(args.eval_only, name_model='model-best.pth.tar')
    #     # trainer.load_model_feat(args.eval_only, name_model='model-best.pth.tar')
    #     trainer.test()
    #
    #     print('Eval with epoch 200')
    #     trainer.load_model(args.eval_only, name_model='model.pth.tar-200')
    #     # trainer.load_model_feat(args.eval_only, name_model='model.pth.tar-200')
    #     trainer.test()
    #
    #     print('Eval with best AUC')
    #     trainer.load_model(args.eval_only, name_model='model-best-auc.pth.tar')
    #     # trainer.load_model_feat(args.eval_only, name_model='model-best-auc.pth.tar')
    #     trainer.test()
    #
    #     print('Eval with best balance')
    #     trainer.load_model(args.eval_only, name_model='model-best-balance.pth.tar')
    #     # trainer.load_model_feat(args.eval_only, name_model='model-best-balance.pth.tar')
    #     trainer.test()
    #
    #     print('\nEval with best ACC with AUC > 50')
    #     trainer.load_model(args.eval_only, name_model='model-best-50auc.pth.tar')
    #     # trainer.load_model_feat(args.eval_only, name_model='model-best-50auc.pth.tar')
    #     trainer.test()
    #     return
...
if __name__ == "__main__":
    ...
    parser.add_argument("--viz-attn", type=str, default="None",
                        help="directory path, similar as args.eval_only")
