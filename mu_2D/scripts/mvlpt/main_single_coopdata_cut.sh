...
#root=/project/hnguyen2/stly/code/datasets/prompting/temp
#output_dir=/project/hnguyen2/stly/code/prompting/logs
#root=/project/hnguyen2/stly/code/datasets/prompting/medmnist_ori
#root=/project/hnguyen2/stly/code/datasets/grand_challenges
root=/project/hnguyen2/stly/code/datasets/proposals/proposal_prompt
#root=/project/hnguyen2/stly/code/datasets/wilds_stanford/datasets
output_dir=$9
lr=$6
eval_only=$7
get_feat=${10}
...
#MODEL_PATH=$8
#MODEL_DIR="--model-dir ${MODEL_PATH}"

MODEL_PATH='/project/hnguyen2/stly/code/prompting/pretrained'
MODEL_DIR="--model-dir ${MODEL_PATH}/${PRETRAIN_DATASET}/${TRAINER}/${CFG}_${SHOTS}shots/nctx${NCTX}_csc${CSC}_ctp/"

#if [ "$eval_only" = true ]; then
#  eval_cmd="--eval-only ${root}/${TRAINER}/${CFG}_${SHOTS}shots/nctx${NCTX}_csc${CSC}_ctp/seed${}"
#else
#  eval_cmd="--eval-only False"
#fi
#
#echo $eval_cmd

#eval_cmd="--eval-only /project/hnguyen2/stly/code/proposals/prompts/log_prompts/trial1/Proposal_prompt_0/VPT/vit_b16_1shots/nctx16_csc_ctp/seed1/"
#eval_cmd="--eval-only ${eval_only}"
#echo $eval_cmd
...
do
    ...
    if $viz_attn ; then
      viz_cmd="--viz-attn ${DIR}"
    else
      viz_cmd="--viz-attn None"
    fi
        ...
        python3 train_mvlpt_clip/train_baseline_bias.py \
         ...
         TEST.FINAL_MODEL "best_val"
          ...
          python3 train_mvlpt_clip/train_mvlpt_lora_last.py \
           ...
           TEST.NO_TEST False \
           TEST.FINAL_MODEL "best_val"
    elif  [ $TRAINER = "VPT_lorass_last" ]; then
          python3 train_mvlpt_clip/train_mvlpt_lorass_last.py \
           --root $root \
           --seed ${SEED} \
           --trainer MVLPT_lorass_last_trainer \
           --config-file configs/trainers/MVLPT/${CFG}.yaml \
           --output-dir ${DIR} \
           --dataset ${DATASET} \
           --shots ${SHOTS} \
           ...
           TEST.NO_TEST False \
           TEST.FINAL_MODEL "best_val"
    elif  [ $TRAINER = "Baseline_sb" ]; then
        python3 train_mvlpt_clip/train_baseline_sb.py \
         --root $root \
         --seed ${SEED} \
         --trainer Baseline_sb_trainer \
         --config-file configs/trainers/MVLPT/${CFG}.yaml \
         --output-dir ${DIR} \
         --dataset ${DATASET} \
         --shots ${SHOTS} \
