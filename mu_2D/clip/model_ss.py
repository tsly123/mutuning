...
class ResidualAttentionBlock(nn.Module):
    def __init__(self, d_model: int, n_head: int, mode: str or None, attn_mask: torch.Tensor = None):
        ...
        :param n_head:
        :param attn_mask:
        :param mode: 4 cases: scalebias, qkv, attn, qkvattn
        """
        super().__init__()
        self.mode = mode
        if self.mode is None:
            ...
            if self.mode != 'qkv_only':     # scalebias
    ...
    def attention(self, x: torch.Tensor):
        ...
        if self.mode is None or 'qkv' not in self.mode or self.mode == 'scalebias':  # scalebias
            return self.attn(x, x, x, need_weights=True, attn_mask=self.attn_mask)#[0]

        elif 'qkv' in self.mode:
            ...
            return self.attn(q, k, v, need_weights=True, attn_mask=self.attn_mask)#[0]


    def forward(self, x: torch.Tensor, return_attn_weight=False, return_attention=False):

        if self.mode is None or self.mode == 'qkv_only':
        ...
        elif self.mode != 'qkv_only':   # scalebias
...
class VisionTransformer(nn.Module):
    def __init__(self, input_resolution: int, patch_size: int, width: int, layers: int, heads: int, output_dim: int, ssmode: str or None):
        super().__init__()
        self.input_resolution = input_resolution
        self.output_dim = output_dim
